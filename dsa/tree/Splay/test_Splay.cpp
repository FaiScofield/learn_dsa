...
// 周期性访问测试
template <typename T> void  testSplayPeriod(int n) {
    Splay<T> splay;
...
// 随机访问测试
template <typename T> void  testSplayRandom(int n) {
    Splay<T> splay;
    while (splay.size() < n) {
        ...
        switch (dice (3)) {
            ...
            case 1: { // 删除，成功率 <= 33.3%
                ...
                splay.remove(e) ?
                (void) printf("Removal done\n") :
                (void) print(e), printf(" not exists\n");
                break;
            }
            default: { // 插入，成功率 == 100%
                ...
                splay.insert(e);
                (e == splay.root()->data) ?
                (void) printf("Insertion done with"), print(*(splay.root())), (void) printf("\n") :
                print(e), "duplicated";
                break;
            }
        } // switch
