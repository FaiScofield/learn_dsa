...
struct Vertex {
    ...
    int parent; int priority; // 在遍历树中的父节点、优先级数（最短通路，极短跨边等）

    // 构造新顶点
    Vertex(const Tv& d =(Tv) 0) :
        data(d), inDegree(0), outDegree(0), status(UNDISCOVERED),
        dTime(-1), fTime(-1), parent(-1), priority(WINT_MAX) {} // 暂不考虑权重溢出
};
...
struct Edge {
    ...
    // 构造新边
    Edge() : weight(0), type(UNDETERMINED) {}
...
// 顶点的动态操作
    ...
    virtual int insert(const Tv& vertex) {
        for (int j = 0; j < this->n; j++) E[j].insert((Edge<Te>*) nullptr);
        this->n++; // 各顶点预留一条潜在的关联边
//        Edge<Te>* empty_edge = new Edge<Te>();
        Vector<Edge<Te>*> Ve(this->n, (Edge<Te>*) nullptr);
        // @TODO: problem
        E.insert(Ve); // 创建新顶点对应的边向量
...
// 边的动态操作
    ...
    virtual Te remove(int i, int j) {
        Te eBak = edge(i, j);
        delete E[i][j]; E[i][j] = nullptr; // 备份后删除边记录
        this->e--; V[i].outDegree--; V[j].inDegree--; // 更新边计数与关联顶点的度数
