...
void UniPrint::p(const char* s) { s ? printf("%s", s) : printf("<NULL>", s); }
...
static void printBTree (BTNodePosi(T) bt, int depth, bool isLeftmost, bool isRightmost, Bitmap* leftmosts, Bitmap* rightmosts) {
   ...
   bool parentOK = false; BTNodePosi(T) p = bt->parent;
   if (!p) parentOK = true;
   else for (int i = 0; i < p->child.size(); i++) if (p->child[i] == bt) parentOK = true;
