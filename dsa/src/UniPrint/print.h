...
#include "../Queue/Queue.h" // include List
#include "../Stack/Stack.h" // include Vector
...
//    static void p(HuffChar&);    // Huffman（超）字符
    ...
    template <typename T> static void p(const Vector<T>& V);
    template <typename T> static void p(const List<T>& L);
    template <typename T> static void p(const Stack<T>& S);
    template <typename T> static void p(const Queue<T>& Q);

    template <typename T> static void p(const BinNode<T>& node);
    template <typename T> static void p(const BinTree<T>& bt);
...
//    template <typename T> static void p(PQ_LeftHeap<T>&); // PQ_LeftHeap
    ...
    template <typename Tv, typename Te> static void p(GraphMatrix<Tv, Te>& s);
...
template <typename T> void UniPrint::p(const Vector<T>& V) {
    if (V.empty()) return;
    for (int i=0; i<V.size(); ++i) cout << V[i] << " ";
    cout << endl;
}

template <typename T> void UniPrint::p(const List<T>& L) {
    if (L.empty()) return;
...
template <typename T> void UniPrint::p(const Stack<T>& S) {
    if (S.empty()) return;
    for (int i=0; i<S.size(); ++i) cout << S[i] << " ";
    cout << endl;
}

template <typename T> void UniPrint::p(const Queue<T>& Q) {
    if (Q.empty()) return;
...
// BinTree节点
template <typename T> void UniPrint::p(const BinNode<T>& node) {
    p(node.data); // 数值
/*
    // height & NPL
 ...
 * 二叉树输出打印
 ******************************************************************************************/
template <typename T> void UniPrint::p(const BinTree<T>& bt) {
    if (!bt.size()) return;
 ...
 * 基于BinTree实现的BST
 ******************************************************************************************/
template <typename T> void UniPrint::p(BST<T>& bt) { // 引用
    if (!bt.size()) return;
    printf("%s[%p]*%d:\n", typeid (bt).name(), &bt, bt.size()); // 基本信息
    Bitmap* branchType = new Bitmap; // 记录当前节点祖先的方向
    printBinTree(bt.root(), -1, ROOT, branchType); // 树状结构
 ...
 ******************************************************************************************/
template <typename T> void UniPrint::p(AVL<T>& avl) { // 引用
 ...
 * 其中调用的BinNode的打印例程，可以显示BF状态
 ******************************************************************************************/
template <typename T> void UniPrint::p(RedBlack<T>& rb) { // 引用
 ...
 ******************************************************************************************/
template <typename T> void UniPrint::p(Splay<T> & bt) { // 引用
    printf("%s[%p]*%d:\n", typeid (bt).name(), &bt, bt.size()); // 基本信息
    Bitmap* branchType = new Bitmap; // 记录当前节点祖先的方向
    printBinTree(bt.root(), -1, ROOT, branchType); // 树状结构
    delete branchType;
 ...
 * BTree打印（入口）
 ******************************************************************************************/
template <typename T> void UniPrint::p(BTree<T> & bt) { // 引用
    printf("%s[%p]*%d:\n", typeid (bt).name(), &bt, bt.size()); // 基本信息
    ...
    delete leftmosts; delete rightmosts;
    printf("\n");
}
 ...
 * Entry
 ******************************************************************************************/
template <typename K, typename V> void UniPrint::p(Entry<K, V>& e)
 ...
 * Hashtable
 ******************************************************************************************/
template <typename K, typename V> void UniPrint::p(Hashtable<K, V>& ht) {
...
void UniPrint::p(GraphMatrix<Tv, Te>& g) {
    int inD = 0, outD = 0;
    for (int i = 0; i < g.n; i++) inD += g.inDegree(i);
    for (int i = 0; i < g.n; i++) outD += g.outDegree(i);
    // 基本信息
    printf("%s[%p]*(%d, %d):\n", typeid(g).name(), &g, g.n, g.e);
    // 标题行
    printf("%3d %3d |  ", g.n, inD);
    for (int i = 0; i < g.n; i++) {
        print(g.vertex(i)); printf("[");
        print(g.status(i)); printf("]\t");
    }
    printf("\n");
    // 标题行（续）
    printf("%3d %3d |  ", outD, g.e);
    for (int i = 0; i < g.n; i++) { print(g.inDegree(i)); printf(" \t"); }
    printf("| dTime fTime Parent Weight\n");

    // 水平分隔线
    printf("--------+--"); for (int i = 0; i < g.n; i++) printf("-----------");
    printf("-+--------------------------\n");

    // 逐行输出各顶点
    for (int i = 0; i < g.n; i++) {
        print(g.vertex(i)); printf("["); print(g.status(i)); printf("] "); print(g.outDegree(i)); printf("  |  ");
        for (int j = 0; j < g.n; j++)
            if (g.exists (i, j)) { print(g.edge(i, j)); print(g.type(i, j)); printf("\t"); }
            else printf(".\t");
        printf("|   "); print(g.dTime(i)); printf("    "); print(g.fTime(i)); printf("     ");
        if (0 > g.parent(i)) print("^  "); else print(g.vertex(g.parent(i))); printf(" ");
        if (WINT_MAX > g.priority(i)) print(g.priority(i)); else print(" INF");
        printf("\n");
    }
    printf("\n");
}
 ...
 * 二叉树各种派生类的统一打印
 ******************************************************************************************/
template <typename T> static void printBinTree(BinNodePosi(T) bt, int depth, int type, Bitmap* bType) {
    if (!bt) return;
 ...
 * BTree打印（递归）
 ******************************************************************************************/
template <typename T> static void printBTree(BTNodePosi(T) bt, int depth, bool isLeftmost, bool isRightmost, Bitmap* leftmosts, Bitmap* rightmosts) {
    if (!bt) return;
    ...
    if (!p) parentOK = true;
    else for (int i = 0; i < p->child.size(); i++)
        if (p->child[i] == bt) parentOK = true;
