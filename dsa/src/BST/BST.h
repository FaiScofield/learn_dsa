...
static BinNodePosi(T)& searchIn(BinNodePosi(T)& v, const T& e, BinNodePosi(T)& hot) {
    if(EQUAL(e, v)) return v; // 退化情况：在子树根节点v处命中
    hot = v;
    while(1) { // 一般地，反复不断地
...
// 返回时，返回值指向命中节点（或假想的通配哨兵），hot指向其父亲（退化时为初始值nullptr）
template <typename T>
static BinNodePosi(T) & searchIn_iter(BinNodePosi(T)& v, const T& e, BinNodePosi(T)& hot) {
    if(!v || (e == v->data)) return v; // 递归基：在节点v（或假想的通配节点）处命中
    hot = v; // 一般情况：先记下当前节点，然后再
    return searchIn_iter(((e < v->data) ? v->lc : v->rc), e, hot); // 深入一层，递归查找
}
 ...
 * 目标x在此前经查找定位，并确认非nullptr，故必删除成功；与searchIn不同，调用之前不必将hot置空
 * 返回值指向实际被删除节点的接替者，hot指向实际被删除节点的父亲——二者均有可能是nullptr
 ******************************************************************************************/
template <typename T>
 ...
 *   a   b   c
 * T0  T1  T2  T3
 * 故无论是zig还是zag旋转(单旋或者双旋)，可以直接利用此性质修改其祖先/孩子的链接即可
 * 最后统统变成为:
 *        b
 *      /   \
 *    a       c
...
template <typename T> BinNodePosi(T) BST<T>::connect34 (
    ...
    // print(a); print(b); print(c); printf("\n");
