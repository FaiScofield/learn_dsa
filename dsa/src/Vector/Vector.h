...
protected:
    ...
    // 轴点构造算法(版本C)：通过调整元素位置构造区间[lo, hi]的轴点，并返回其秩
    // O(n)
...
// 构造函数
    ...
    Vector(Rank n) : _capacity(n) { _elem = new T[_capacity]; _size = 0; }
    Vector(Rank n, const T& v) : _capacity(n) { _elem = new T[_capacity]; for (_size=0; _size<n; _elem[_size++]=v); }
    Vector(const T* A, Rank n) { copyFrom(A, 0, n); } // 数组整体复制
    Vector(const T* A, Rank lo, Rank hi) { copyFrom(A, lo, hi); } // 数组区间复制
    Vector(const Vector<T>& V) { copyFrom(V._elem, 0, V._size); } // 向量整体复制
    Vector(const Vector<T>& V, Rank lo, Rank hi) { copyFrom(V._elem, lo, hi); }  // 向量区间复制
...
// 只读访问接口 const
    ...
    Rank fibSearch(T* A, const T& e, Rank lo, Rank hi) const {
        ...
        while (lo < hi) {                               // 每步迭代仅仅做一次比较判断，有两个分支
            ...
            while (hi - lo < cFib.get()) cFib.prev();   // 通过向前顺序查找（分摊O(1)）——至多迭代几次？
    ...
    Rank search(const T& e, Rank lo, Rank hi) const {
        ...
        return rand() % 2
               ? this->binSearch(_elem, e, lo, hi)
               : this->fibSearch(_elem, e, lo, hi);
    }
...
// 可写访问接口
    ...
    T& operator [] (Rank r) const { assert(0 <= r && r < _size); return _elem[r]; }
    // 默认作为末元素插入
    ...
    // 对向量区间[lo, hi)排序
    void sort(Rank lo, Rank hi, int method=3) {
        switch (method) {
...
// 遍历
    // 遍历（使用函数指针，只读或局部性修改）
    // 借助函数指针机制遍历向量
    void traverse(void (*) (T&)) { for (int i=0; i<_size; ++i) visit(_elem[i]); }

    // 遍历（使用函数对象，可全局性修改）
    // 借助函数对象机制遍历向量
    template <typename VST> void traverse(VST&) { for (int i=0; i<_size; ++i) visit(_elem[i]); }
}; // Vector
