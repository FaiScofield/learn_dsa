...
template <typename T> void PQ_ComplHeap<T>::heapify(Rank n) {
    // 自上而下的上滤. Sigma(depth(i)) = O(nlogn)
    // for (int i = 1; i < n; ++i) percolateUp(i);

    // 自底而上的下滤. Sigma(height(i)) = O(n)
    for (int i = LastInternal(n); InHeap(n, i); i--) {
        percolateDown(n, i);
//        for (int k = 0; k < n; k++) {
//            int kk = k;
//            while (i < kk) kk = (kk - 1) / 2;
//            i == kk ? print(this->_elem[k]) : print("    ");
//        }
//        printf("\n");
    }
}
