...
{  // 在[0, 2n)中选择n个偶数，随机插入列表
    ListNodePosi(T) p =
        rand() % 2 ? list.insertAsLast(rand() % (T)n * 2) : list.insertAsFirst(rand() % (T)n * 2);
    for (int i = 1; i < n; i++)
        p = rand() % 2 ? list.insertBefore(p, rand() % (T)n * 2) : list.insertAfter(p, rand() % (T)n * 2);
}
...
void testList(int testSize)
{
    ...
    printf("\n  ==== Test %2d. Remove redundancy (ordered) in\n", testID++);
    PRINT(La);
    printf("%d node(s) removed\n", La.uniquify());
    PRINT(La);
    printf("\n  ==== Test %2d. Remove redundancy (disordered) in\n", testID++);
    PRINT(Le);
    printf("%d node(s) removed\n", Le.deduplicate());
    PRINT(Le);
    printf("\n  ==== Test %2d. Sort\n", testID++);
