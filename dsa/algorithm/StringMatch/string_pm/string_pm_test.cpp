...
int main ( int argc, char* argv[] ) {
   ...
   for ( int i = 0; i < ( argc - 1 ) / 2 * 2; i += 2 ) {
      system ( "cls" ); printf ( "####################################################\n" );
      printf ( "Text:     " ); printString ( argv[i+2] ); printf ( "\n" );
      printf ( "Pattern:  " ); printString ( argv[i+1] ); printf ( "\n" );
      int pos = match ( argv[i+1], argv[i+2] );
      if ( strlen ( argv[i+2] ) < pos + strlen ( argv[i+1] ) )   printf ( "No matching found\a\a\n" );
      else                                               printf ( "Matching found at #%d\a\n", pos );
      getchar();
   }
   return 0;
}
